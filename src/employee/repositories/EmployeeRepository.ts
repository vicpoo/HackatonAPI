...
export class EmployeeRepository {
  ...
  public static async findAll(): Promise<Employee[]> {
    return new Promise((resolve, reject) => {
      connection.query('SELECT * FROM employee', (error: any, results) => {
        if (error) {
          reject(error);
        } else {
  ...
  public static async createEmployee(employee: Employee): Promise<Employee> {
    const query = 'INSERT INTO employee (full_name, role_id_fk, created_at, created_by, updated_at, updated_by, deleted) VALUES (?, ?, ?, ?, ?, ?, ?)';
    console.log(employee);
    return new Promise((resolve, reject) => {
      connection.execute(query, [employee.full_name, employee.role_id_fk, employee.created_at, employee.created_by, employee.updated_at, employee.updated_by, employee.deleted], (error, result: ResultSetHeader) => {
        if (error) {
          reject(error);
        } else {
