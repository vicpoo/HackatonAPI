...
export class OrderRepository {
  ...
  public static async findAll(): Promise<Order[]> {
    return new Promise((resolve, reject) => {
      connection.query('SELECT order_id, date_orders, client_id_fk FROM orders', (error, results) => {
        if (error) {
          reject(error);
        } else {
  ...
  public static async findById(order_id: number): Promise<Order | null> {
    return new Promise((resolve, reject) => {
      connection.query('SELECT order_id, date_orders, client_id_fk FROM orders WHERE order_id = ?', [order_id], (error, results) => {
        if (error) {
          reject(error);
        } else {
          const orders: Order[] = results as Order[];
  ...
  public static async createOrder(order: Order): Promise<Order> {
    const query = 'INSERT INTO orders (date_orders, client_id_fk) VALUES (?, ?)';
    return new Promise((resolve, reject) => {
  ...
  public static async updateOrder(orderId: number, order: Order): Promise<void> {
    const query = 'UPDATE orders SET date_orders = ?, client_id_fk = ? WHERE order_id = ?';
    return new Promise((resolve, reject) => {
